class User < ActiveRecord::Base
	...
	has_many :posts, through: :relationships, dependent: :destroy

	email_regex = /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]+)\z/i

	validates :name, presence: true, length: { maximum: 60 }
  
	validates :alias, presence: true, length: { maximum: 60 }
	...
	# like / unlike methods
	def like(post)
		relationships.create(post_id: post.id)
	end

	def unlike(post)
		relationships.find_by(post_id: post.id).destroy
	end
